...
/**
 * Rewrite the WordPress gallery shortcode
 * with data attributes for Fancybox3.
 *
...
class Controlled_Chaos_Gallery_Shortcode {
	...
	public function gallery_shortcode( $attr, $content, $id ) {
		...
		$atts  = apply_filters( 'ccd_fancybox_atts', shortcode_atts( [
			// Default parameters.
			'order'      => 'ASC',
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			$fancybox = sprintf( 'data-type="image" data-fancybox="gallery" title=""' );
			if ( ! empty( $atts['link'] ) && 'file' === $atts['link'] ) {
				$image_output = sprintf( '<a class="%1s-link" href="%2s" data-type="image" data-fancybox="group-%3s"%4s>%5s</a>', $selector, wp_get_attachment_url( $id ), $selector, $caption, wp_get_attachment_image( $id, $atts['size'], false, $attr ) );
...
$gallery_shortcode = new Controlled_Chaos_Gallery_Shortcode;